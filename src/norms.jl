...
`l2norm(ps)` follows from `GeometricBase`'s generic `l2norm(x) = sqrt(L2norm(x))`, which is the
quantity the callers want: the blocks of a parameter set combine **in quadrature**. Summing their
norms instead overestimates the ℓ² norm by up to ``\sqrt{k}`` for ``k`` blocks, and thereby every
stopping criterion computed from it.
...
`abs2(l2norm(x))` and not `L2norm(x)` at each leaf, which is the same asymmetry from the other side:
`L2norm` is what a leaf would have to override to be counted correctly, and a structured leaf overrides
`l2norm`. It costs a `sqrt` and a square per leaf, which is a rounding step and not a term.
